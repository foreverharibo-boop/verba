...
// in that bounded 3 x 3 case actually start instead of leaving B/C queued
// behind the stalled A requests they are supposed to outrun.
...
function profileRaceTimeoutError(minutes = normalizedProfileRaceTimeoutMinutes()) {
    ...
    error.code = 'VERBA_PROFILE_RACE_TIMEOUT';
    return error;
}
...
async function translateOutputText(source, options = {}) {
    ...
    const speakerIdentity = options.speakerIdentity || {};
    const speakerScopes = inferLocalTargetDialogueScopes(segmented, speakerIdentity);
    ...
    // Rebuild omitted or malformed source+Korean pairs locally instead of
    // spending another AI request on formatting repair.
    for (const segment of segmented.segments) {
...
function injectSettingsPanel() {
    ...
    panel.innerHTML = `
        <div class="inline-drawer">
            ...
            <div class="inline-drawer-content" style="display: none;">
                ...
                <div class="verba-help">설정한 대기 시간이 지나도 응답이 끝나지 않으면 다음 프로필도 겹쳐 호출하고, 먼저 정상 완료된 번역만 사용해요. 프로필 자동 사용과 서로 다른 프로필 두 개 이상이 필요합니다.</div>
                <div id="verba-profile-race-options" class="verba-profile-race-options" ${settings.profileRaceEnabled === true && settings.autoProfileFallback !== false ? '' : 'hidden'}>
                    <label for="verba-profile-race-stagger-seconds">다음 프로필 동시 요청 대기</label>
                    <div class="verba-profile-race-time-row">
                        <input type="number" inputmode="numeric" min="5" max="180" step="1" id="verba-profile-race-stagger-seconds" class="text_pole" value="${normalizedProfileRaceStaggerSeconds()}">
                        <span>초</span>
                    </div>
                    <div class="verba-help">A 요청 후 입력한 시간이 지나면 B를, 다시 같은 시간이 지나면 C를 시작해요. 명확한 일시적 오류가 나면 기다리지 않고 다음 프로필을 즉시 호출합니다.</div>
                    <label for="verba-profile-race-timeout-minutes">전체 강제 종료 시간</label>
                    <div class="verba-profile-race-time-row">
                        <input type="number" inputmode="decimal" min="1" max="1440" step="1" id="verba-profile-race-timeout-minutes" class="text_pole" value="${normalizedProfileRaceTimeoutMinutes()}">
                        <span>분</span>
                    </div>
    ...
    const profileRaceStaggerInput = panel.querySelector('#verba-profile-race-stagger-seconds');
