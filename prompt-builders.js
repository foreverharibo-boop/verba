...
export function createPromptBuilders(h) {
    ...
    function galbwae(s = {}, scope = 'mixed') {
        ...
        return lines([
            ...
            '- MODE=all: apply it to Korean narrative prose (row type="narration"), every Korean direct-dialogue target, and eligible visible natural-language text enclosed by Markdown **...**. Never apply it to any paired-tag interior. For selection rows, apply when in_tagged_content is false.',
            '- MODE=dialogueInner: apply it ONLY to every Korean direct-dialogue target and eligible visible natural-language text enclosed by Markdown **...**. Keep other narrative prose normally spelled. Never apply it to any paired-tag interior. For selection rows, apply only when in_dialogue is true or the selected text is enclosed by ** delimiters in the supplied context.',
            '- NEVER apply it to visible text inside ANY paired <tag>...</tag>, including <Inner_Info>, <Info_panel>, <small>, <div> and custom tags. Also never apply it to dates, weather, locations, ordinary metadata/tag text, code, attributes, URLs, numbers, protected tokens, or already-Korean-rendered proper names. Keep Korean-rendered proper names and particles attached directly to those names normally spelled.',
            ...
            '- First understand the meaning, then REWRITE every eligible sentence in chaotic 죠캎-style Korean internet-post language. The voice should feel strangely earnest, overexcited and clumsily typed: awkwardly polite wording may collide with casual wording, emotional emphasis and ridiculous-looking phrasing. Mix in a LIGHT, intermittent internet-grandpa flavor, as if a slightly confused old man is typing in an online community. Do not turn the whole response into historical-drama speech or repeat generic ~느냐/~거라/~로다 endings.',
            '- Visibly wreck spelling and spacing across the sentence: phonetic misspellings, swapped vowels/consonants, wrong-but-readable particles/endings, words stuck together, spaces inserted in odd places, doubled or missing syllables, and conspicuous community-post punctuation. Useful mutation patterns include 어떻게→어덕헤, 계속→개속, 진짜→진자, 했는데→햇는대, 주세요→주새요, 있어요→잇어요, 거예요→거애요. Sprinkle ㄷㄷ, ;; and ㅠㅠ where the emotion permits, but never attach all of them to every sentence. Vary everything instead of mechanically applying the same substitution.',
            '- Every eligible sentence must be unmistakably transformed while remaining decipherable. Do not output clean modern Korean with one corrupted 받침, and do not merely convert every ending to one repeated catchphrase. Preserve the original scene content: 죠캎-style absurdity belongs to wording only, so never invent new actions, body parts, sexual content, incidents, objects or claims.',
            '- MARKDOWN IS FORMATTING, NOT A TEXT EXEMPTION: preserve Markdown delimiters, nesting and placement, but translate and apply the selected GALBWAE mode to eligible visible natural-language text between **...**. This rule overrides any generic instruction to preserve Markdown. Other Markdown formatting stays structurally preserved, and inline/fenced code enclosed by backticks remains completely unchanged. Example: **Do you know me?** → **나를 아늕랴!!**.',
            '- PAIRED TAGS ARE AN ABSOLUTE GALBWAE EXEMPTION: preserve every opening/closing tag, attribute, tag order and normally spelled translated inner text. Do NOT apply GALBWAE to visible text between ANY <tag>...</tag> pair. There are no tag-name exceptions.',
            '- REQUIRED PROFANITY MUTATION: whenever an eligible Korean rendering would naturally use 씨발, NEVER output the clean spelling 씨발. Choose and vary among 씨핤, 씨핧, 샤갈, 쌱앐, 쌰갈, 시핣. Preserve the profanity target, function and situational intensity; do not add it where profanity is not licensed by the active GALBWAE style.',
            '- OCCASIONAL ENDING/REPLY MUTATION: irregularly change some sentence-final 요 to 료 (for example 알겠어요→알갰어료, 한다네요→한다내료), and occasionally change a standalone acknowledgement 네 or 응 to 례. Apply these only to a minority of eligible opportunities—roughly one out of three, with uneven spacing—not to every 요, 네 or 응 and never inside paired tags.',
            '- Preserve facts, speaker, intent, emotional direction, profanity target/strength, relationship direction, quotation marks, ellipses, name identity, numbers and protected tokens. Character names must appear in natural Hangul without GALBWAE misspelling, and their attached particles remain grammatically correct. For eligible 갈봬체 only, ? and ! may be exaggerated or replaced when the rewritten ending still makes the speech act unmistakable; preserve ellipsis characters/count/order exactly.',
            ...
            '- STYLE EXAMPLES (pattern examples, not text to copy): "나 알아?" → "나를 아늕랴!!"; "알겠어요." → "알갰어료 ㅠㅠ"; "네, 그렇게 할게요." → "례.. 그러캐할개료"; "응, 알겠어." → "례 알갯다내료;;"; "씨발, 뭐야?" → "쌰갈 머냐고요 ㄷㄷ"; "계속 보고 있었어요." → "개속 보고잇엇내요"; "도와주세요." → "도아주새요 ㅠㅠ"; "왜 그러는 거야?" → "왜그러는건대요????"',
            `- CURRENT SCOPE=${scope}.`,
        ]);
    }
        ...
        if (exclusiveGalbwae) {
            return lines([
                ...
                scope==='tagged_content'
                    ? 'TAGGED CONTENT: translate visible Korean-target natural language only. Preserve tag structure and metadata layout. GALBWAE is forbidden for every paired-tag interior, including inner_info; keep all translated tagged text normally spelled.'
    ...
    function repair(kind,segments,translations,s={},i={},tokens=[],over=null,scope='mixed') {
        ...
        const task={tokens:'Restore expected_protected_tokens exactly at their source-implied positions/counts. Never expose or invent tokens; copy every other correct word unchanged.',banned:'Replace detected banned words with natural wording of matching intent/force; do not merely delete them or alter anything else.',untranslated:'Translate accidental foreign leftovers only; retain correct Korean and register. If detected_problem starts with UNTRANSLATED_CHARACTER_NAME, verify each listed token from source context: a human or fictional character name MUST be rendered in natural Hangul (fixed name mapping first), never retained in Latin script merely because it is a proper name. Retain Latin spelling only for intentional bilingual output or genuine non-person acronyms, brands, products, codes, URLs and handles.'}[kind];
